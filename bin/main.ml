...
(* let rec get_type (scope: state list) (name: Ast.id) : Types.typ =
  match scope with
  | [] -> Stdlib.failwith ("Name(" ^ name ^ ") Not found")
  | x :: xs -> if Hashtbl.mem x name then (match Hashtbl.find x name with
  | typ, _ -> typ) else get_type xs name *)


(* type state = (Ast.id, Types.typ * (Ast.expr option)) Hashtbl.t *)
...
(* todo: check on function-return-types so that `type_of_value` only needs to check function's ret_type rather than evaluating and creating side-effects *)
(* let (*rec*) type_of_value (e: Ast.expr) : Types.typ = match e with 
| Ast.Bool b -> Types.TBool
| Ast.Int b -> Types.TInt
| BinaryOp(bop, e1, e2) -> raise (Stdlib.Failure "must pass evaluated value") (* type_of_value (eval_expr e) *)
| Name n -> raise (Stdlib.Failure "must pass evaluated value") type_of_value (eval_expr e) *)
...
(* let _ =
  print_string "hello";
  print_string (dump_f (parse (read_whole_file "bin/test.owo"))) *)

let _ =
  let globals: state = Hashtbl.create 1021 in
    let scope: state list = [globals] in
      exec_f scope (parse (read_whole_file "bin/test.owo"))